...
class LeftSideCampaignSlider extends PageBuilderBase
{

    public function preview_image(): string
    {
        return 'campaign/campaign-slider-style-one';
    }
    ...
    public function frontend_render(): string
    {
        ...
        $campaign = Campaign::with(["product" => function ($query){
            ...
            $query->with("campaign_sold_product","campaign_product","taxOptions:tax_class_options.id,country_id,state_id,city_id,rate","vendorAddress:vendor_addresses.id,country_id,state_id,city_id");
            ...
            $query = productCampaignCondition($query);
            return $query;
        ...
        if($campaign){
            $campaign->product = $campaign?->product->isNotEmpty() ? $campaign->product?->transform(function ($item) {
        ...
        return $this->renderBlade("campaign/style-01", compact("campaign","section_title","padding_bottom", "padding_top"));
    }
